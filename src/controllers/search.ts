...
import { IPaginateProps, ISearchResult } from "@harshbhama/jobber-shared";
...
export async function singleGigById(req: Request, res: Response): Promise <void> {
  const gig = await gigById('gigs', req.params.gigById);
  res.status(StatusCodes.OK).json({ messsage: 'Single gig result', gig});
}